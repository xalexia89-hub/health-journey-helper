...
export const PreventiveAdvisorChat = () => {
  ...
  const gatherContext = async () => {
    if (!user) return {};
    const [nutrition, activity, sleep, stress, medical, health] = await Promise.all([
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const aiContent = error ? 'Λυπάμαι, δεν μπόρεσα να επεξεργαστώ το αίτημά σας. Δοκιμάστε ξανά.' : data.message;
      ...
      const { data: errMsg } = await supabase.from('preventive_chat_messages').insert({
        user_id: user.id,
        role: 'assistant',
        content: 'Παρουσιάστηκε σφάλμα. Παρακαλώ δοκιμάστε ξανά.',
      }).select().single();
      ...
      <Card className="bg-gradient-to-br from-primary/10 to-accent/5 border-primary/20">
        <CardContent className="p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-semibold">Προληπτικός Σύμβουλος</span>
          </div>
          <p className="text-xs text-muted-foreground">
            Αναλύω τα δεδομένα υγείας σας και προτείνω βελτιώσεις τρόπου ζωής. Δεν αντικαθιστώ γιατρό.
          </p>
        </CardContent>
      </Card>
        ...
        <div className="space-y-3">
          {messages.length === 0 && (
            <div className="text-center py-8 text-muted-foreground text-sm">
              ...
              <p>Ρωτήστε με για διατροφή, ύπνο, στρες ή πρόληψη!</p>
